...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <div className="app-frame"><aside className="app-nav"><Brand /><span className="nav-label">WORKSPACE</span><Link href="/app/dashboard">Dashboard</Link><Link href="/app/cases">Cases</Link><Link href="/app/workspace">Variant workspace</Link><span className="nav-label">GOVERNANCE</span><Link href="/app/review">Review queue</Link><Link href="/app/reports">Reports</Link><Link href="/app/audit">Audit</Link><Link href="/app/settings">Settings</Link><button className="text-button" onClick={() => firebaseAuth && signOut(firebaseAuth)}>Sign out</button></aside><main className="app-content">{!firebaseConfigured && <div className="configuration-notice">Development mode: Firebase client configuration is absent. Production requires Firebase authentication and server-side membership provisioning.</div>}{children}</main></div>;
}
