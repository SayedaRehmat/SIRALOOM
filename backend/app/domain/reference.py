...
class EnsemblReference:
    ...
    def __init__(
        ...
        *,
        ...
        if build not in {"GRCh37", "GRCh38"}:
            raise ReferenceError(f"Unsupported remote reference build: {genome_build}")
    ...
    def _fetch_remote(self, contig: str, start1: int, end1: int) -> str:
        if start1 < 1 or end1 < start1:
            raise ReferenceError("Invalid 1-based Ensembl reference interval")
        ...
        try:
            ...
            raise ReferenceError(f"Ensembl reference request failed with HTTP {exc.code}: {url}") from exc
        except URLError as exc:
            raise ReferenceError(f"Unable to reach Ensembl reference service: {exc}") from exc
        except TimeoutError as exc:
            raise ReferenceError("Ensembl reference request timed out") from exc
        sequence = "".join(data.split()).upper()
        if not sequence:
            raise ReferenceError(f"Ensembl returned an empty sequence for {url}")
        if any(base not in "ACGTN" for base in sequence):
            raise ReferenceError("Ensembl returned unexpected reference sequence characters")
        expected_length = end1 - start1 + 1
        if len(sequence) != expected_length:
            raise ReferenceError(f"Ensembl returned {len(sequence)} bases, expected {expected_length}")
        return sequence
    ...
    def fetch(self, contig: str, start0: int, end0: int) -> str:
        ...
        if relative_start < 0 or relative_end > len(sequence):
            raise ReferenceError("Remote reference cache window does not cover requested interval")
    ...
    def contig_length(self, contig: str) -> int:
        raise ReferenceError("Remote Ensembl reference does not provide contig length through the local-reference interface")
